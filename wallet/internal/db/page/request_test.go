...
// TestMapRows verifies that mapRows converts rows via toItem, propagates
// converter errors immediately, and returns errNilItem on a (nil, nil) return.
func TestMapRows(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "converts all rows successfully",
			rows: []int{1, 2, 3},
			toItem: func(row int) (*int, error) {
				doubled := row * 2
		...
		{
			name: "converter error stops early",
			rows: []int{1, 2, 3},
			toItem: func(row int) (*int, error) {
				if row == 2 {
					return nil, errTest
		...
		{
			name: "toItem returning (nil, nil) returns errNilItem",
			rows: []int{1},
			toItem: func(row int) (*int, error) {
				// intentionally return (nil, nil) for test.
				//nolint:nilnil
				return nil, nil
			},
			wantItems: nil,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			items, err := mapRows(tc.rows, tc.toItem)
			...
			require.Equal(t, tc.wantItems, items)
		})
	}
}
